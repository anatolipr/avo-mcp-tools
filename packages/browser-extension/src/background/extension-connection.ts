...
// extension becomes a channel," independent of any page's connection. One
// connection for the whole install (not per-tab), analogous to a page's own
// TenantConnection but living in the background service worker.
//
...
import { getOrCreateExtensionAppLabel, getExtensionChannel } from './storage.js';
...
export async function startExtensionConnection(): Promise<void> {
  const [appLabel, channel] = await Promise.all([getOrCreateExtensionAppLabel(), getExtensionChannel()]);
  const bus = getExtensionToolBus();

  socket = connectStateSocket(
    {
      onConnect() {
        console.log(`[browser-extension] connected to js-bridge-mcp as "${appLabel}" on channel "${channel}"`);
        socket!.send({
          type: 'register_tools',
    ...
    { serverUrl: JSBRIDGE_HOST, tenant: channel }
  );
