...
export function registerMemoryTools(mcp: McpServer, repo: MemoryRepository): void {
  ...
  mcp.tool(
    'memory_get',
    'Exact-match lookup of memory docs (plan/spec/sql/etc.) by normalized key — a ticket ID or a free-form name like "Spot Chart Design". Returns every doc under that key, or only the matching doc_type if provided. If no doc\'s key matches, falls back to a filename-substring match (e.g. a ticket ref that only appears in the filename, not the key) — the response includes a `filename_fallback: true` note when this happened. Paused docs are hidden by default — pass include_paused to see them.',
    {
      key: z.string(),
    ...
    async ({ key, doc_type, include_paused }) => {
      const exact = repo.getByKey(key, doc_type, { includePaused: include_paused });
      if (exact.length > 0) {
        return { content: [{ type: 'text', text: JSON.stringify(exact, null, 2) }] };
      }
      const fallback = repo.getByFilenameContains(key, doc_type, { includePaused: include_paused });
      const result = fallback.length > 0 ? fallback.map((doc) => ({ ...doc, filename_fallback: true })) : fallback;
  ...
  mcp.tool(
    'memory_bulk_get',
    'Fetches many memory docs by (folder, filename) in one call, including full markdown bodies — e.g. hydrating a batch of memory_search hits. Missing docs are simply omitted from the result, not errors.',
    { refs: z.array(z.object({ folder: z.string().optional(), filename: z.string() })).min(1) },
    async ({ refs }) => {
      const docs = await repo.bulkGet(refs);
  ...
  mcp.tool(
    'memory_bulk_update',
    'Applies the same frontmatter change to many memory docs at once by (folder, filename) — e.g. add/remove a tag across a batch found via memory_search, or mark a group of docs "shipped". add_tags/remove_tags merge or subtract per-doc; status/related_to overwrite uniformly when provided. Body is never touched. Returns per-doc success/failure so one bad entry doesn\'t abort the batch.',
    {
      refs: z.array(z.object({ folder: z.string().optional(), filename: z.string() })).min(1),
    ...
    async ({ refs, add_tags, remove_tags, status, related_to, deprecated }) => {
      const results = await repo.bulkUpdate(refs, { add_tags, remove_tags, status, related_to, deprecated });
  ...
  mcp.tool(
    'memory_create',
    `Writes a new memory doc (plan, spec, SQL, testing notes, discovery, etc.) into the memory folder under the given key. Pick a descriptive filename (e.g. "IRT-123-Fix-for-language.md"); a doc-type suffix like -PLAN/-DISCOVERY/-SPEC/-NOTES/-SESSION-SUMMARY helps when a key has multiple docs. ${AUTHORING_SKILL_HINT}`,
    {
      filename: z.string().describe('on-disk filename, e.g. "IRT-123-Fix-for-language.md" (.md appended if omitted); auto-suffixed on collision'),
      ...
      related_to: z.string().optional().describe('key of a related doc, e.g. a spec linking to its plan\'s key'),
    ...
    async ({ filename, key, key_type, doc_type, description, body, tags, status, related_to, subfolder, folder }: any) => {
      try {
        ...
        const doc = await repo.create({ filename, key, key_type, doc_type, description, body, tags, status, related_to, subfolder, folder });
  ...
  mcp.tool(
    'memory_bulk_create',
    `Writes many memory docs in one call — each entry is the same shape as memory_create's args. Returns per-key success/failure (with the new filename on success) so one bad entry doesn't abort the rest of the batch. ${AUTHORING_SKILL_HINT}`,
  ...
  mcp.tool(
    ...
    async ({ folder, filename, body, body_edits, ...frontmatterFields }: any) => {
      ...
      try {
        ...
        if (body_edits) {
          const existing = await repo.get(folder, filename);
        ...
        const doc = await repo.update(folder, filename, frontmatterFields, body);
        ...
        // Fetch memory_get(key) if the fresh full body is actually needed.
  ...
  mcp.tool(
    'memory_rename',
    'Renames a memory doc\'s file in place (frontmatter/content and any attachments unchanged) — use when a doc was created with an unclear filename and should be given a better one.',
    { ...docRefSchema, new_filename: z.string().describe('the new on-disk filename, e.g. "IRT-123-Fix-for-language.md"') },
    async ({ folder, filename, new_filename }: any) => {
      try {
        const doc = await repo.rename(folder, filename, new_filename);
        const { body: _omitted, ...docWithoutBody } = doc;
        return { content: [{ type: 'text', text: JSON.stringify(docWithoutBody, null, 2) }] };
      } catch (err) {
        return { content: [{ type: 'text', text: (err as Error).message }], isError: true };
      }
    }
  ...
  mcp.tool(
    'memory_set_paused',
    'Pauses or resumes memory docs — a local-only toggle stored in this machine\'s SQLite cache, never written to the doc\'s markdown file, so it never touches the file on disk and never follows the doc to another machine\'s cache. Paused docs are hidden from memory_get/memory_search by default but stay fully intact and are still fetchable directly via memory_bulk_get — use this to temporarily stop a doc from surfacing during discovery without deprecating it. Returns per-doc success/failure so one bad entry doesn\'t abort the batch.',
    {
      refs: z.array(z.object({ folder: z.string().optional(), filename: z.string() })).min(1),
      paused: z.boolean().describe('true to pause (hide from memory_get/memory_search), false to resume'),
    },
    async ({ refs, paused }) => {
      const results = await repo.setPaused(refs, paused);
      return { content: [{ type: 'text', text: JSON.stringify(results, null, 2) }] };
    }
  );
  ...
  mcp.tool(
    'memory_delete',
    'Hard-deletes a memory doc — removes the markdown file, no tombstone.',
    docRefSchema,
    async ({ folder, filename }: any) => {
      try {
        await repo.delete(folder, filename);
        return { content: [{ type: 'text', text: `Deleted memory doc "${filename}"` }] };
      } catch (err) {
        return { content: [{ type: 'text', text: (err as Error).message }], isError: true };
      }
    }
  ...
  mcp.tool(
    'memory_bulk_delete',
    'Hard-deletes many memory docs in one call — e.g. cleaning up a batch of abandoned docs found via memory_search. No tombstone. Returns per-doc success/failure so one bad entry doesn\'t abort the rest of the batch.',
    { refs: z.array(z.object({ folder: z.string().optional(), filename: z.string() })).min(1) },
    async ({ refs }) => {
      const results = await repo.bulkDelete(refs);
      return { content: [{ type: 'text', text: JSON.stringify(results, null, 2) }] };
    }
  );
  ...
  mcp.tool(
    'memory_save_session',
    `Saves a summary of the current chat session as a memory doc (doc_type "session-summary"). Pass a summary, not a raw transcript. If key or description are omitted, ask the user for them rather than guessing; filename defaults to a slug of key+description if omitted. ${AUTHORING_SKILL_HINT}`,
    {
      summary: z.string().describe('a scannable summary of the session, not a raw transcript'),
      key: z.string().optional(),
      description: z.string().optional(),
      filename: z.string().optional().describe('defaults to a slug of key+description if omitted'),
      tags: z.array(z.string()).optional(),
      ...(multiFolder ? { folder: z.string().optional().describe(`which configured memory folder to write into: ${folderNames}`) } : {}),
    },
    async ({ summary, key, description, filename, tags, folder }: any) => {
      if (!key || !description) {
      ...
      try {
        const doc = await repo.create({
          filename: filename ?? `${slugify(key)}-${slugify(description)}`,
          key,
