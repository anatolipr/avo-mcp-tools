...
/**
 * Generic toast/notification system, ported from htmlpaint.com's Toast.svelte +
 * notifications.js (a top-center stack, capped at a few visible, auto-dismissing on a timer,
 * themed by type). Transport is a `window`-level CustomEvent (`mem-bucket-toast`), not a shared
 * module import — dispatching a real DOM event (bubbles + composed, so it crosses shadow-DOM
 * boundaries) means ANY element, anywhere in the tree, including ones that never import this
 * file, can trigger a toast just by dispatching the event shape below. `toast.success(...)` etc.
 * are sugar over exactly that dispatch, for the common case of already being in a .ts file that
 * can import this module — both call paths are equivalent, pick whichever is more convenient.
 * Mount ONE <toast-stack> near the app root (see mem-bucket-app.ts); it's the only listener.
 */
...
/** The `detail` shape of a `mem-bucket-toast` CustomEvent — `type`/`timeoutMs` are optional so a
 ...
 * equivalent to dispatching a `mem-bucket-toast` CustomEvent by hand (see TOAST_EVENT). */
export const toast = {
  send: dispatch,
...
/** Mount exactly one of these (e.g. in mem-bucket-app.ts's render()) — it listens for
 * `mem-bucket-toast` on `window` regardless of where in the tree it's placed. */
