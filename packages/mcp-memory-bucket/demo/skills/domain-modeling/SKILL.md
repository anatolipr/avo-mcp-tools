...
description: >-
  Actively build and sharpen a project's domain model: challenge fuzzy or
  conflicting terminology against the existing glossary, invent edge-case
  scenarios to stress-test relationships between concepts, cross-reference
  stated behavior against the actual code, and capture resolved terms in
  CONTEXT.md and hard-to-reverse decisions as ADRs. Use when discussing codebase
  terminology, writing or editing a CONTEXT.md, or recording or editing an ADR —
  not just reading existing docs for vocabulary.
tags:
  - grill
  - domain-modeling
  - adr
